...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        for (int i = 0; i < iters.size(); i++) {
            ...
            if (iter.hasNext()) {
                Pair<Record, Integer> pair = new Pair<>(iter.next(), i);
                pq.add(pair);
            }
        }
        while (pq.size() > 0) {
            Pair<Record, Integer> popped = pq.poll();
            int runIndex = popped.getSecond();
            merged.add(popped.getFirst());
            if (iters.get(runIndex).hasNext()) {
                Pair<Record, Integer> pair = new Pair<>(iters.get(runIndex).next(), runIndex);
                pq.add(pair);
            }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        for (int i = 0; i < runs.size(); i += numBufs) {
            if (i+numBufs > runs.size()) {
                mergedRuns.add(mergeSortedRuns(new ArrayList<>(runs.subList(i, runs.size()))));
            } else {
                mergedRuns.add(mergeSortedRuns(new ArrayList<>(runs.subList(i, i+numBufs))));
            }
        }
        return mergedRuns;
    }
    ...
    public Run sort() {
        ...
        while (sourceIterator.hasNext()) {
            Iterator<Record> currIter = getBlockIterator(sourceIterator, getSource().getSchema(), numBuffers);
            Run currRun = sortRun(currIter);
            sortedRuns.add(currRun);
        }
