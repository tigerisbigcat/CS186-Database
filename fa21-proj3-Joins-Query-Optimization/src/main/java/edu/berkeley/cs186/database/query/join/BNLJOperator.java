...
public class BNLJOperator extends JoinOperator {
    ...
    private class BNLJIterator implements Iterator<Record>{
        ...
        private Record fetchNextRecord() {
            ...
            Record rightRecord;
            if (leftRecord == null) {
                //left source empty, nothing to fetch
                return null;
            }
            while (true) {
                if (rightPageIterator.hasNext()) {
                    //there's a next right record in page, join it if matches
                    rightRecord = rightPageIterator.next();
                    if (compare(leftRecord, rightRecord) == 0) {
                        return leftRecord.concat(rightRecord);
                    }
                } else if (leftBlockIterator.hasNext()) {
                    //no next right record in page, but there is left record in block. reset right page.
                    leftRecord = leftBlockIterator.next();
                    ...
                    //no next right record or left record, but there is next right page.
                    //reset left block and advance right page.
                    fetchNextRightPage();
                    if (!rightPageIterator.hasNext()) //will this ever happen?
                        return null;
                    rightRecord = rightPageIterator.next();
                    ...
                    //no next right record/left record and no next right page, but there is next left block.
                    //reset right source and advance left block.
...
//            else if (leftSourceIterator.hasNext()) {
//                fetchNextLeftBlock();
//                Record leftRecord = leftBlockIterator.next();
//                rightPageIterator.reset();
//                return leftRecord;
//            }
//            else {
