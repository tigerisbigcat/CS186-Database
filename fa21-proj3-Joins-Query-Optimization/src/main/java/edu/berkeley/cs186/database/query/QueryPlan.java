...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int indexCol = -1;
        for (Integer i: indexCols) {
            SelectPredicate currPred = this.selectPredicates.get(i);
            IndexScanOperator currOp = new IndexScanOperator(this.transaction, table, currPred.column, currPred.operator, currPred.value);
        ...
        minOp = addEligibleSelections(minOp, indexCol);
        return minOp;
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        // check each set of tables in prevMap
        for (Set s : prevMap.keySet()) {
            for (int i = 0; i < joinPredicates.size(); i++) {
                String[] leftColNames = new String[joinPredicates.size()];
                leftColNames[i] = joinPredicates.get(i).leftColumn;
                String[] rightColName = new String[joinPredicates.size()];
                rightColName[i] = joinPredicates.get(i).rightColumn;

                // Get the left side and the right side of the
                // predicate (table name and column)
                String leftTable = leftColNames[0];
                String leftCol = leftColNames[1];

                String rightTable = rightColName[0];
                String rightCol = rightColName[1];
                ...
                if (s.contains(leftTable) && !s.contains(rightTable)) {
                    rightSet.add(leftTable);
                    QueryOperator leftOp = prevMap.get(s);
                    QueryOperator rightOp = pass1Map.get(rightSet);

                    //  Using the operator from Case 1 or 2, use minCostJoinType to
                    //  calculate the cheapest join with the new table (the one you
                    //  fetched an operator for from pass1Map)
                    minOp = minCostJoinType(leftOp, rightOp, leftCol,
                            rightCol);
                }
                ...
                //  to fetch an operator to access the leftTable.
                    ...
                    minOp = minCostJoinType(leftOp, rightOp, leftTable+'.'+leftCol,
                            rightTable+'.'+rightCol);
                }
                ...
                if (!result.containsKey(newSet)) {
                    ...
                    if (minOp.estimateIOCost() < result.get(newSet).estimateIOCost()) {
                        result.put(newSet, minOp);
                    }
                }
            }
