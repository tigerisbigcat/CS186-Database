...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        ...
        // leaf.scanAll() return rids.iterator() in LeafNode class.
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        LeafNode rootNode = root.get(key);
        return new BPlusTreeIterator(rootNode, key);
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor)
     throws BPlusTreeException {
        ...
        if (leftNode != this.root || leftNode.scanAll().hasNext()) {
            throw new BPlusTreeException("empty, can't load into a empty tree");
        }

        // when the tree is not empty, add the pairs to the tree.
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> op = this.root.bulkLoad(data, fillFactor);
            if (op.isPresent()) {
                Pair<DataBox, Long> pair = op.get();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(LeafNode leaf) {
            this.leaf = leaf;
            this.iterator = leaf.scanAll();
        }

        public BPlusTreeIterator(LeafNode leaf, DataBox key) {
            this.leaf = leaf;
            this.iterator = leaf.scanGreaterEqual(key);
        }

        @Override
        public boolean hasNext() {

            // if it's not empty, return ture
            if (iterator.hasNext()) {
                return true;
            }

            // if it's empty, check the right siblings. if has right siblings, return true.
            else {
            ...
            // if not the 2 cases above, return false.
            return false;
        }
        ...
        public RecordId next() {
            ...
            // if iterator not empty, return next one
            if (iterator.hasNext()) {
                return iterator.next();
            }

            // if it's empty, check the right siblings. If it has right siblings, return next one
            else {
                while (leaf.getRightSibling().isPresent()) {
                    leaf = leaf.getRightSibling().get();
                    iterator = leaf.scanAll();
            ...
            // if it's not the 2 cases above, throw exception.
