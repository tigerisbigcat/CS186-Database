...
public class PageDirectory implements BacktrackingIterable<Page> {
    ...
    public Page getPageWithSpace(short requiredSpace) {
        ...
        // When we modify a page, we'll almost always end up reading it
        // first (acquiring IS/S locks) and then write back our updates
        // to it afterwards (promoting to IX/X locks). If we know ahead
        // of time that we're going to modify a page, we can skip the IS/S
        // locks altogether by just acquiring IX/X locks to begin with.
        // Modify the following methods to request the appropriate lock upfront
