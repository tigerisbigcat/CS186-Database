...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                // TODO(proj4_part2)
                // get the current locks from lock manager
                List<Lock> currLocks = lockManager.getLocks(this);

                // reverse the order to release
                Collections.reverse(currLocks);

                for (Lock l : currLocks) {

                    // only use LockContext#release and not LockManager#release
